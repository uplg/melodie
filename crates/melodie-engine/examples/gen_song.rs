...
fn write_wav(path: &str, wav: &Tensor) -> Result<()> {
    ...
    for i in 0..ch0.len() {
        for &s in &[ch0[i], ch1[i]] {
            wtr.write_sample((s.clamp(-1.0, 1.0) * 32767.0) as i16)
...
fn main() -> Result<()> {
    ...
    let codes = {
        ...
        let muq = if std::env::var("MELODIE_NOMUQ").is_ok() {
            None
        } else {
            Some(p.muq_idx)
        };
    ...
    let cdev = if std::env::var("MELODIE_CPU_CODEC").is_ok() {
        Device::Cpu
    } else {
        dev.clone()
    };
