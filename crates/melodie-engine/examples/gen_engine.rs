...
fn main() -> Result<()> {
    let frames: usize = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(400);
    ...
    let opts = GenOptions { max_frames: frames, ..Default::default() };
    let audio = engine.generate(&lyrics, TAGS, &opts)?;
    std::fs::write("gen_engine.wav", audio.to_wav_bytes()?)?;
    println!("wrote gen_engine.wav ({:.2} s, {} ch)", audio.duration_secs(), audio.channels);
    Ok(())
}
